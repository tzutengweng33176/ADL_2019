...
def get_random_label(batch_size, hair_classes, hair_prior, eye_classes, eye_prior, face_classes, face_prior, glass_classes, glass_prior):
    ...
    face_code = torch.zeros(batch_size, face_classes)  # One hot encoding for hair class
    glass_code = torch.zeros(batch_size, glass_classes)  # One hot encoding for hair class
    ...
    face_type = np.random.choice(face_classes, batch_size, p = face_prior)  # Sample eye class from eye class prior
    glass_type = np.random.choice(glass_classes, batch_size, p = glass_prior)  # Sample eye class from eye class prior
    #https://docs.scipy.org/doc/numpy-1.14.0/reference/generated/numpy.random.choice.html  
    #it's optional to give hair_prior abd eye_prior
    #print(hair_type.shape) # 0~5 #(128,)
    #print(eye_type.shape) # 0~3 #(128,)
    #print(face_type.shape) #0~2 #(128,)
    #print(glass_type.shape) #0 or 1  #(128,)
    #input()
    for i in range(batch_size):
        hair_code[i][hair_type[i]] = 1
        eye_code[i][eye_type[i]] = 1
        face_code[i][face_type[i]]=1
        glass_code[i][glass_type[i]]=1

    return torch.cat((hair_code, eye_code, face_code, glass_code), dim = 1) 

def generation_by_attributes(model, device, batch_size ,latent_dim, hair_classes, eye_classes, face_classes, glass_classes,
    ...
    glass_tag = torch.zeros(batch_size, glass_classes).to(device)
    ...
    glass_class = np.random.randint(glass_classes)

    for i in range(batch_size):
    	hair_tag[i][hair_class], eye_tag[i][eye_class], face_tag[i][face_class], glass_tag[i][glass_class]= 1, 1, 1, 1

    tag = torch.cat((hair_tag, eye_tag, face_tag, glass_tag), 1)
    z = torch.randn(batch_size, latent_dim).to(device)

    output = model(z, tag)
    if step is not None:
        file_path = '{}_hair_{}_eyes_{}_face_{}_step_{}.png'.format(hair_mapping[hair_class], 
                eye_mapping[eye_class],face_mapping[face_class], glass_mapping[glass_class], step)
    else:
        file_path = '{}_hair_{}_eyes_{}_face_{}.png'.format(hair_mapping[hair_class], eye_mapping[eye_class],
                face_mapping[face_class], glass_mapping[glass_class])
