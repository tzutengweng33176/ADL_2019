...
parser.add_argument('-b', '--batch_size', help = 'Training batch size', #try batch size 8
                    default = 32, type = int)
...
def main():
    ...
    classification_criterion= torch.nn.NLLLoss() #NLLLoss does not support multi-target (?????)
...
######################################################################################################        
        ...
        #-log(P(class=c|X_real))
        #print(real_discrim_loss)
        #print(real_classifier_loss)

        errD_real= real_discrim_loss + real_classifier_loss
        #print(errD_real)
        errD_real.backward()
        #print(errD_real)
        #input()
        ...
        #print(fake_discrim_loss)
        fake_classifier_loss= criterion(fake_predict, fake_tag)
        #print(fake_classifier_loss)
        errD_fake= fake_discrim_loss + fake_classifier_loss
        #print(errD_fake)
        errD_fake.backward()
        #print(errD_fake)

        classifier_loss= real_classifier_loss + fake_classifier_loss
...
#################################################################################################
        ...
        if step_i == 1:
            save_image(denorm(real_img[:25,:,:,:]), os.path.join(random_sample_dir, 'real.png'), nrow=5)
        ...
        if step_i % args.sample == 0:
            save_image(denorm(fake_img[:25,:,:,:]), os.path.join(random_sample_dir, 'fake_step_{}.png'.format(step_i)), nrow=5)
