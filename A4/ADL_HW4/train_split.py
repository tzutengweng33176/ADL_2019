...
def main():
    ...
    config = 'ACGAN_split_smooth=0.7_*1.0_20190522_100-batch_size-[{}]-steps-[{}]'.format(batch_size, iterations)
...
#for more detail, see https://github.com/gitlimlab/ACGAN-PyTorch/blob/master/main.py
    for step_i in range(1, iterations + 1):
        ...
        fake_score, _, _, _, _ = D(fake_img.detach()) #you need detach() here
        ...
        fake_discrim_loss = criterion(fake_score, fake_soft_label) #-log(1-P(S=fake|x_fake))
        ...
        discrim_loss = (real_discrim_loss + fake_discrim_loss)*1.0  #Ls in the original paper; or adversarial loss
        ...
        G_loss = classifier_loss_G*args.classification_weight + discrim_loss_G 
        #G_optim.zero_grad()
        G_loss.backward()
        G_optim.step() #fix D, update G
