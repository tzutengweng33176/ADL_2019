...
pickle_in_x = open("x_pg_no_baseline.pkl","rb")
pickle_in_y = open("y_pg_no_baseline.pkl","rb")
x_099 = pickle.load(pickle_in_x)
y_099 = pickle.load(pickle_in_y)

pickle_in_x_ddqn = open("x_pg_with_baseline.pkl","rb")
pickle_in_y_ddqn = open("y_pg_with_baseline.pkl","rb")
...
plt.plot(x_099, y_099, label='No baseline')
plt.plot(x_ddqn, y_ddqn, label='Add baseline')
...
plt.ylabel('Avg Reward in the last 1 episodes')
plt.title('Avg reward of PG with and without baseline',fontweight='bold', fontsize=16 )
#new_ticks = np.linspace(0, 10, 100)
#print(new_ticks)
#plt.yticks(new_ticks)

plt.savefig('PG_with_without_b.png')
