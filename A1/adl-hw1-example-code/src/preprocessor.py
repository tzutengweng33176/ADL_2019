...
class Preprocessor:
    ...
    def sentence_to_indices(self, sentence):
        ...
        indices=[]
        for word in tokens:
            #print("index: ", self.embedding.to_index(word))
    ...
    def collect_words(self, data_path, n_workers=4):
        ...
        #print(utterances[0])  #utterace of speaker 1
        #print(utterances[1])  
        #print(utterances[2])
        #print(utterances[3]) 
        #print(utterances[4])
        #print(utterances[5])
        utterances = list(set(utterances)) #remove repeated utterances and make the list unordered
        #print(utterances[0])
        #print(utterances[1])
        #print(utterances[2])
        #print(utterances[3])
        #print(utterances[4])
        #print(len(utterances)) #89801 for test set
        #input()
        chunks = [
            ' '.join(utterances[i:i + len(utterances) // n_workers])  #from i to i+len//4
            for i in range(0, len(utterances), len(utterances) // n_workers)
        ]
        #print(chunks[0])
        with Pool(n_workers) as pool:
            chunks = pool.map_async(self.tokenize, chunks)
            #print("chunks.get(): ", len(chunks.get()[2])) #len(chunks.get())=5 ;
            #chunks is a list of lists
            words = set(sum(chunks.get(), [])) #sum() is concatenating lists

        return words  #return a set of words
    ...
    def preprocess_sample(self, data):
        ...
        if 'options-for-correct-answers' in data:
            ...
            processed['n_corrects'] = 0 #correct answer will always be the first one
