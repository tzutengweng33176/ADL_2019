...
def main(args):
    ...
    with open(config_path) as f:
        config = json.load(f) #config is a Python dictionary
    ...
    words |= preprocessor.collect_words(config['test_json_path'],
                                        n_workers=args.n_workers)  #|= is union operation
    ...
    embedding = Embedding(config['embedding_vec_path'], words)  #where to get pretrained word embedding-->word2vec, fastText, GloVe; cannot use contexualized word embeddings
