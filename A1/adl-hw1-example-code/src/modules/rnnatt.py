...
class RNNAtt(torch.nn.Module):
    ...
    def forward(self, context, context_lens, options, option_lens):
        ...
        for i, option in enumerate(options.transpose(1, 0)):
            ...
            #print("attn[0].shape: ", attn[0].shape)
            #plt.matshow(attn[0].cpu().numpy())
        ...
        #print("len(attention): ", len(attention)) #total number of batches
        #print("len(attention[0]):", len(attention[0])) #batch_size
        #print(attention[0].shape) #torch.Size([batch_size, opt_len, context_len])
