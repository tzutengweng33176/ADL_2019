...
class ExamplePredictor(BasePredictor):
    ...
    def __init__(self, embedding,
        ...
        self.embedding = torch.nn.Embedding(embedding.size(0),
                                            embedding.size(1))
        self.embedding.weight = torch.nn.Parameter(embedding) #load pretrained embedding
        #print("embedding.size: ", embedding.size()) #torch.Size([83842, 300]), where 83842 is the size of the dictionary of embeddings; 300 is the embedding dimension
        # use cuda
    ...
    def _run_iter(self, batch, training):
        with torch.no_grad():
            ...
            #print("batch['context']: ", batch['context']) #a tensor of indices
            options = self.embedding(batch['options'].to(self.device))
            #print("batch['options']: ", batch['options']) #a tensor of indices
